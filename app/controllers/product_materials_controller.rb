class ProductMaterialsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @product_material.save
        ...
        format.json { render action: 'show', status: :created, location: @product_material }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      if params[:returnUrl]
        format.html { redirect_to product_materials_path(params[:returnUrl]) }
      else
        format.html { redirect_to product_materials_url }
      end
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def product_material_params
      params.require(:product_material).permit(:product_id, :material_id, :g, :oz)
    end
end
